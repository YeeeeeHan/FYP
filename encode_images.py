...
def main():
    ...
    parser.add_argument('--data_dir', default='models', help='Directory for storing optional models')
    ...
    parser.add_argument('--load_effnet', default='models/finetuned_effnet.h5', help='Model to load for EfficientNet approximation of dlatents')
    ...
    for images_batch in tqdm(split_to_batches(ref_images, args.batch_size), total=len(ref_images)//args.batch_size):
        ...
        if args.use_best_loss:
            print(f"@@@@ if args.use_best_loss, best_dlate: {best_dlatent.shape}")
            generator.set_dlatents(best_dlatent)
        generated_images = generator.generate_images()
        print(f"@@@@ Generated_images details: {type(generated_images)}, {generated_images.shape}")
        generated_dlatents = generator.get_dlatents()
        for img_array, dlatent, img_path, img_name in zip(generated_images, generated_dlatents, images_batch, names):
            print(f"@@@@ img_array: {img_array.shape}, img_path: {img_path}, img_name: {img_name}")
            mask_img = None
            if args.composite_mask and (args.load_mask or args.face_mask):
                ...
                print(f"@@@@ mask_img: {mask_img}, im_name: {im_name}")
